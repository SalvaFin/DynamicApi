...
public class UserCodeDirectoryRepository : IUserCodeDirectoryRepository
{
    ...
    {
        string normalizedUserCode = userCode.Trim().ToUpperInvariant();
        return _dbContext.UserCodeDirectoryEntries.FirstOrDefaultAsync(
            entry => entry.UserCode.Trim().ToUpper() == normalizedUserCode,
            cancellationToken);
    }
